...
int process (string inFile, int MaxEvents, int dEvents, int targMass, int iSim, bool printCuts, int bMixEvt) {
    ...
    MassDifference myMassDiff;

    srand(time(NULL)); // initial seed for random number generator
    myMixEvt.Put_MixingMethod(bMixEvt); // set the mixing method
    myMixEvt.Put_NumberOfEventsToMix(5); // add number of mixed event iterations
    int NUM_ENTRIES_OFFSET = myMixEvt.Get_NumberOfEventsToMix(); // retreive number of mixed event iterations
    vector<int> MixEvtNumbers;
    ...
    double MASS_OMEGA = myHistManager.GetMASS_OMEGA();
    ...
    PartReader pipMixEvt(myTree,"PiPlus");
    PartReader pimMixEvt(myTree,"PiMinus");
    PartReader photon1MixEvt(myTree,"Photon1");
    PartReader photon2MixEvt(myTree,"Photon2");
...
//        myHistManager.GetStartTime()->Fill(eventStartTime);
        ...
        yDIS = nu/beam.E(); // fraction of electron's initial energy lost in the target rest frame
        ...
        myMixEvt.Put_LorentzVector(photon2Reader.GetLorentzVector(MASS_PHOTON),"Photon2",0);
        myMixEvt.Put_LorentzVector(pipReader.GetLorentzVector(MASS_PION_CHARGED),"Pi+",0);
        myMixEvt.Put_LorentzVector(pimReader.GetLorentzVector(MASS_PION_CHARGED),"Pi-",0);
        myMixEvt.Reconstruct_Pi0(0);
        myMixEvt.Reconstruct_Omega(0);

        for(k=0; k<=NUM_ENTRIES_OFFSET; k++){
            MixEvtNumbers.clear(); // delete vector contents and set size to 0
            // create the out-of-time particle objects
            if(bMixEvt==myMixEvt.Get_Index("Random Mixing")){
              MixEvtNumbers.push_back(rand()%entries); // event number for pi+
              MixEvtNumbers.push_back(rand()%entries); // event number for pi-
              MixEvtNumbers.push_back(rand()%entries); // event number for photon 1
              MixEvtNumbers.push_back(rand()%entries); // event number for photon 2
            }else{
              iMixedEvt = rand()%entries; // random number between 0 and entries
              MixEvtNumbers.push_back(iMixedEvt); // event number for pi+
              MixEvtNumbers.push_back(iMixedEvt); // event number for pi-
              MixEvtNumbers.push_back(iMixedEvt); // event number for photon 1
              MixEvtNumbers.push_back(iMixedEvt); // event number for photon 2
            }

            pipMixEvt.ReadEntry(MixEvtNumbers.at(0));
            pimMixEvt.ReadEntry(MixEvtNumbers.at(1));
            photon1MixEvt.ReadEntry(MixEvtNumbers.at(2));
            photon2MixEvt.ReadEntry(MixEvtNumbers.at(3));

            myMixEvt.Put_InTimeEvt(processed);
            myMixEvt.Put_OutOfTimeEvt(MixEvtNumbers);

            // add the out-of-time particle 4-vectors
            myMixEvt.Put_LorentzVector(pipMixEvt.GetLorentzVector(MASS_PION_CHARGED),"Pi+",1);
            myMixEvt.Put_LorentzVector(pimMixEvt.GetLorentzVector(MASS_PION_CHARGED),"Pi-",1);
            myMixEvt.Put_LorentzVector(photon1MixEvt.GetLorentzVector(MASS_PHOTON),"Photon1",1);
            myMixEvt.Put_LorentzVector(photon2MixEvt.GetLorentzVector(MASS_PHOTON),"Photon2",1);
            myMixEvt.Reconstruct_Pi0(1);
            myMixEvt.Reconstruct_Omega(1);

            myMixEvt.Mix_Omega(); // run mixing routine for each method

            // retrieve the mixed event 4-vectors
            TLorentzVector nPion = myMixEvt.Get_LorentzVector("Pi-",2); // pi- Lorentz vector
            TLorentzVector pPion = myMixEvt.Get_LorentzVector("Pi+",2); // pi= Lorentz vector
            TLorentzVector photon1 = myMixEvt.Get_LorentzVector("Photon1",2); // photon 1 Lorentz vector
            TLorentzVector photon2 = myMixEvt.Get_LorentzVector("Photon2",2); // photon 2 Lorentz vector
            TwoPhoton = myMixEvt.Get_LorentzVector("Pi0",2); // Two photon Lorentz vector
            Omega = myMixEvt.Get_LorentzVector("Omega",2); // omega meson Lorentz vector
            TwoPion = pPion + nPion; // pion pair Lorentz vector

            pipReader.ReadEntry(myMixEvt.Get_EvtNum("Pi+"));
            pimReader.ReadEntry(myMixEvt.Get_EvtNum("Pi-"));
            photon1Reader.ReadEntry(myMixEvt.Get_EvtNum("Photon1"));
            photon2Reader.ReadEntry(myMixEvt.Get_EvtNum("Photon2"));
            ...
            TLorentzRotation tlrOmega(Omega.BoostVector());
            TLorentzRotation tlrOmega_inv = tlrOmega.Inverse();
            Omega_RF = Omega;
            Omega_RF.Transform(tlrOmega_inv);
            nPion_RF = nPion;
            nPion_RF.Transform(tlrOmega_inv);
            pPion_RF = pPion;
            pPion_RF.Transform(tlrOmega_inv);
            TwoPhoton_RF = TwoPhoton;
            TwoPhoton_RF.Transform(tlrOmega_inv);

            inPlane_3pions = TwoPhoton_RF.Vect().Dot(nPion_RF.Vect().Cross(pPion_RF.Vect()));
            ...
            cuts_PipPim = Analyze_PipPim(pimReader, pipReader, pionPlot); // charged pion pair analysis

            cuts_Electron = false; // initialize the electron cut
            cuts_Electron = Analyze_Electron(elecReader, Qsq, targMass, iSim, elecPlot); // electron analysis

            cuts_Photons = false; // initialize the photon pair cut
            cuts_Photons = Analyze_Photons(photon1Reader, photon2Reader,iSim, gamPlot); // photon pair analysis
            ...
            if(cuts_Electron && cuts_Photons && cuts_PipPim){
                ...
                if(myCuts.GetCut_OmegaID()){
                    ...
                    chPionAngle = TMath::RadToDeg()*pPion.Angle(nPion.Vect());
                    myHistManager.GetOpAngChPions_VS_IMOmega_AllCuts(Vz_index)->Fill(chPionAngle, Omega.M()); // variable = angular difference between 2 charged pions
                    myHistManager.GetOpAngChPions_VS_E_AllCuts(Vz_index)->Fill(TwoPion.E(), chPionAngle); // variable = angular difference between 2 charged pions

                    myHistManager.GetMsq_ChPions_VS_TwoPhotons_AllCuts(Vz_index)->Fill(TwoPhoton.M2(), TwoPion.M2());

                    myHistManager.GetyDIS_VS_IMOmega_AllCuts(Vz_index)->Fill(yDIS, Omega.M()); // variable = DIS kinematics y, fraction of electron energy lost

                    myHistManager.GetDalitz_pip_AllCuts(Vz_index)->Fill(pipPi0.M2(),TwoPion.M2());
                    myHistManager.GetDalitz_pim_AllCuts(Vz_index)->Fill(pimPi0.M2(),TwoPion.M2());
                    myHistManager.GetDalitz_pi0_AllCuts(Vz_index)->Fill(pipPi0.M2(),pimPi0.M2());

                    myCuts.SetDalitz_Parent(MASS_OMEGA);
                    myCuts.SetDalitz_Daughter(MASS_PION_NEUTRAL,1);
                    myCuts.SetDalitz_Daughter(MASS_PION_CHARGED,2);
                    myCuts.SetDalitz_Daughter(MASS_PION_CHARGED,3);
                    myCuts.SetCut_Dalitz(pipPi0.M2(),TwoPion.M2());

                    if(myCuts.GetCut_Dalitz()){
                        myHistManager.GetDalitz_pip_AllCuts_test(Vz_index)->Fill(pipPi0.M2(),TwoPion.M2());
                    ...
                    myMassDiff.Put_Mass(Omega.M(),"Omega");
                    myMassDiff.Put_Mass(pPion.M(),"Pi+");
                    myMassDiff.Put_Mass(nPion.M(),"Pi-");
                    myMassDiff.Put_Mass(TwoPhoton.M(),"Pi0");
                    myMassDiff.Put_Mass(TwoPion.M(),"Pi+Pi-");
                    myMassDiff.Put_Mass(pipPi0.M(),"Pi+Pi0");
                    myMassDiff.Put_Mass(pimPi0.M(),"Pi-Pi0");
                    // set particle squared masses in the MassDifference object
                    myMassDiff.Put_MassSq(Omega.M2(),"Omega");
                    myMassDiff.Put_MassSq(pPion.M2(),"Pi+");
                    myMassDiff.Put_MassSq(nPion.M2(),"Pi-");
                    myMassDiff.Put_MassSq(TwoPhoton.M2(),"Pi0");
                    myMassDiff.Put_MassSq(TwoPion.M2(),"Pi+Pi-");
                    myMassDiff.Put_MassSq(pipPi0.M2(),"Pi+Pi0");
                    myMassDiff.Put_MassSq(pimPi0.M2(),"Pi-Pi0");
            ...
            } // end electron, photon, pion cout
            tlrOmega.Delete(); // delete the omega-meson TLorentzRotation object
            tlrOmega_inv.Delete(); // delete the inverse omega-meson TLorentzRotation object
            //
            // End omega ID
            //
            Plot_Pi0_Decay(TwoPhoton); // plots to check special relativistic kinematics
        } // end for loop for mixing events
...
bool Analyze_Photons(PartReader photon1Reader, PartReader photon2Reader, int iSim, bool bPlot)
{
    ...
    if(myPhotID.GetCut_Photon_All()) myCounter.Increment("Photon ID (All)");
...
bool Analyze_Electron(PartReader elecReader, double Qsq, double targMass, int iSim, bool bPlot)
{
    ...
    if(myElecID.GetCut_ElecECoverP()) myCounter.Increment("Electron ID (ECPvsP)");
    if(myElecID.GetCut_ElecECfid()) myCounter.Increment("Electron ID (ECfid)");
    if(myElecID.GetCut_Elec_All()) myCounter.Increment("Electron ID (All)");
    ...
    if(bPlot){
      ...
      if(myElecID.GetCut_ElecECoverP()) myHistManager.GetECtotP_VS_P_ECPCut(Sector_index-1)->Fill(emMom,emECtot/emMom);
...
void PrintUsage(char *processName)
{
    ...
    cerr << "\t-x#\t\tcreate mixed events (def. = 0, off).\n";
...
int main (int argc, char **argv) {
    ...
    int bMixEvt = 0; // create mixed events
    ...
    while ((c = getopt(argc,argv, "o:M:D:T:S:ix:h")) != -1 ) {
        switch (c) {
            ...
            case 'x': bMixEvt = atoi(optarg); break;
            case 'h':
                PrintUsage(argv[0]);
                exit(0);
                break;
    ...
    for (i = optind; i < argc; ++i) {
        ...
        if (inFile != '-') { // we have a file to process
            ...
            TotEvents = process(inFile,MaxEvents,dEvents,targMass,iSim,printCuts,bMixEvt);
